      ...
      <Card className="mb-6">
        ...
        <div className="mb-8 pb-8">
          {detailKeys.map(({ label, key }) => (
            <div
              key={key}
              className="flex justify-between border-b border-ink-light mt-2 p-1"
            >
              <p className="text-sm text-ink capitalize">{label}</p>
              <p className="text-ink font-medium">{String(booking[key])}</p>
            </div>
          ))}
        </div>
        {/* Price Breakdown */}
        <div className="mb-8 p-4 bg-teal/40 rounded-xl">
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-ink font-medium">{formatPrice(0.85)}</span>
          </div>
